...
void layout::on_accept_clicked()
{
    ...
    qreal indent = ui->firstLineIndent->isChecked() ? ui->changeIndent->value() : 0;
	emit sendLayout( alignment , lineSpacing , margin , indent );
	this->hide();
}

void layout::on_firstLineIndent_stateChanged()
{
    if(ui->firstLineIndent->isChecked())
        ui->changeIndent->setEnabled(true);
    else
        ui->changeIndent->setDisabled(true);
}
